...
parkingRouter.delete("/unpark/:slotNumber", async (req, res) => {
	...
	try {
		const slot = await ParkingLotModel.findOne({
...
parkingRouter.get("/info/:input", async (req, res) => {
	...
	if (!input) {
		return res
			.status(400)
			.send("Input (either car number or slot number) is required.");
	}
	...
	try {
		...
		if (!isNaN(input)) {
			...
			return res
				.status(200)
				.send({ carNumber: slot.carNumber, slotNumber: slot.slotNumber });
		} else {
